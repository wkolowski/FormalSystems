...
(*
#[export] Instance Open_Tm : Open nat Atom Tm := open'.
...
Proof.
  all: induction t; cbn; intros;
    [now decide_all | now decide_all | now rewrite IHt; try congruence | now rewrite IHt1, IHt2].
Defined.

Definition Fresh' (x : Atom) (t : Tm) : Prop :=
  t {{ 0 <~ x }} = t.

(* Lemma 2.4 from the paper *)
Lemma Fresh'_invariant_aux :
  forall (x : Atom) (t : Tm) (i j : nat),
    t {{ j <~ x }} = t <-> t {{ i <~ x }} = t.
Proof.
...
Lemma Fresh'_invariant_aux' :
  forall (x : Atom) (t : Tm) (i : nat),
    t {{ i <~ x }} = t <-> forall j : nat, t {{ j <~ x }} = t.
Proof.
  split; [| easy].
...
Proof.
  intros; unfold Fresh'.
  now apply Fresh'_invariant_aux.
Qed.
...
Proof.
  unfold lci.
  induction 1; intros j Hle.
  - now cbn.
  - cbn; f_equal.
    specialize (Hcof (fresh l) (fresh_spec l)).
    inversion Hcof; subst.
Abort.
...
Proof.
  ...
  induction 1; intros j Hle.
  - now exists x.
  - destruct (H (fresh l) (fresh_spec l) j Hle) as [a Heq].
    exists a; cbn; f_equal.
    admit.
  - 
*)
